...
//找从s到t的最短路径
void Dijkstra(struct Graph G, int s, int t)
{
    struct node *d = (struct node *)malloc(sizeof(struct node) * G.vexnum);
    FibHeap fheap;

    //从s开始搜索
    d[s].has_visited = true;
    d[s].weight = 0;
    d[s].fnode = fheap.insert(0, s);
    //初始化
    ...
    for (int _ = 0; _ < G.vexnum; _++)
    {
        ...
        {
            if (!d[p->vex].has_visited && p->weight + d[min_i].weight < d[p->vex].weight)
            {
                d[p->vex].weight = p->weight + d[min_i].weight;
                fheap.decrease(d[p->vex].fnode, d[p->vex].weight);
            }
        }
    }

    if (d[t].weight < INT_MAX)
        printf("%d\n", d[t].weight);
    else
        printf("-1\n");
    return;
